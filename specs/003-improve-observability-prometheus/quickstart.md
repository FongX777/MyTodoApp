...
- **Kibana server is not ready yet**: This message typically means Kibana is still waiting for Elasticsearch to be fully operational. Check Elasticsearch logs with `docker compose -f docker-compose.observability.yml logs elasticsearch`. Allow up to 5 minutes for the entire stack to initialize properly.
- **Backend fails with database connection errors**: The backend uses the `DATABASE_URL` environment variable to connect to the database. Ensure the PostgreSQL settings in docker-compose.observability.yml match the connection string in the environment variables. After making changes, rebuild and restart the stack: `docker compose -f docker-compose.observability.yml down && docker compose -f docker-compose.observability.yml up --build`
